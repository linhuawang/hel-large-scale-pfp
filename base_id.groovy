...
randomSeed                  = Integer.valueOf(args[3])
...
data.randomize(new Random(randomSeed))
...
for (instance in test) {
  ...
  // printf '%s\n',instance.toString(test.attribute(idAttribute))
   String id = instance.stringValue(test.attribute(idAttribute)) 
   double prediction
    if (!regression) {
...
//    printf '%s,%s\n',instance.toString(test.attribute(idAttribute)),label
...
for (currentNestedFold in 0..nestedFoldCount - 1) {
    ...
    nestedTrain = train.trainCV(nestedFoldCount, currentNestedFold, new Random(randomSeed))
   ...
   //     int id = instance.value(nestedTest.attribute(idAttribute))
        String id = instance.stringValue(nestedTest.attribute(idAttribute))
        double prediction
        if (!regression) {
            label = (instance.stringValue(instance.classAttribute()).equals(predictClassValue)) ? 1 : 0
            prediction = filteredClassifier.distributionForInstance(instance)[predictClassIndex]
